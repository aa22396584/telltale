...
`audited_sha` is required 40 lowercase hex. `--known-sha` is how a test
proves a listed SHA exists; CI checkouts are shallow, so the CLI does not
`git cat-file` history.
