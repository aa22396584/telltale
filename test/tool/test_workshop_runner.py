...
class RunTaskTest(unittest.TestCase):
    ...
    def test_review_refuses_overlapping_dir_lease(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            tmp = Path(raw)
            extra = [
                {
            ...
            dirs = run_task._task_writable_dirs(
                json.loads(plan.read_text(encoding="utf-8"))["tasks"][1]
            )
            lease = run_task._dir_lease_path(tmp, dirs[0])
    ...
    def test_flutter_json_reporter_empty_stdout_is_truncated(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
...
class CampaignRunnerTest(unittest.TestCase):
    ...
    def test_campaign_optional_only_evidence_cannot_complete(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            tmp = Path(raw)
            plan = _plan(
                tmp,
