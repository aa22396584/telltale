...
| `LOOKUP(value:default:key=val:…)` | n-ary colon | Wiki numeric table. Exact `=` and inclusive range `~` (`0~3=4`). First matching pair wins; no match uses `default`. Empty default is 0. Every pair is parsed even after an earlier match (`LOOKUP(A:0:1=100:not-a-pair)` is `unparsableTerm`). Unselected mapped expressions are not reduced (`LOOKUP(A:0:1=1/(A-2):2=200)` of 2 is 200, not `divisionByZero`; `LOOKUP(A:0:1=LOG10(A-2):2=200)` of 2 is 200, not `log10NonPositiveArgument`). Grouped value `LOOKUP((A-1):0:0=50)` is accepted. Quoted strings (`'x'`) are display-side PID substitution, not this engine — `unparsableTerm`. Fewer than one mapping pair (`LOOKUP(A:0)`) is `unparsableTerm`. `2LOOKUP(…)` is not a call. Comma is not a separator. Not `CLOSEST`. |
