...
| `CLOSEST(value:default:key=val:…)` | n-ary colon | Wiki nearest numeric key. `CLOSEST(A:0:1=100)` of 2 is 100; `LOOKUP(A:0:1=100)` of 2 is 0. A distance tie keeps the earlier pair (`128` between `1` and `255` keeps `1`). Default is unused when any pair exists. Grouped value is accepted. Quoted strings are `unparsableTerm`. `CLOSEST(A:0)` is `unparsableTerm`. `2CLOSEST(…)` is not a call. Comma is not a separator. Not `LOOKUP`. |
