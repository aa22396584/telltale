...
3. **打 tag 之前要先落地的兩份紀錄。** 兩者都在 build 之前被檢查，沒有就擋下
   tag —— 用四十分鐘 build 換一個一秒就能給的拒絕，是另一種浪費。

   **(a) 實機走查（只有正式版要）。** 依 §0.1 決定 tag 帶不帶預發行後綴。要發
   正式版，先把
   ...
   `tool/release/require_device_walk.sh` 會查它。完整規則、以及那一行**沒有**
   宣稱的兩件事，見 §0.1。

   **(b) `CHANGELOG.md` 的 `## <版本>` 區段（預發行也要）。**
   `tool/release/release_notes.sh changelog` 會查它，而且這一段會被原樣放進
   GitHub release 內文的**最前面**。`v1.0.13-beta.1` 找的是 `## 1.0.13`。

   什麼**不算數**（每一條都會擋下 tag，訊息各自不同）：

   | 情況 | 為什麼 |
   |---|---|
   | `## Unreleased` | 那底下的東西按定義不在這個 build 裡 |
   | 柵欄程式碼區塊裡的標題 | 對讀者是範例。gate 被自己的文件滿足過一次了（見 §0.1） |
   | HTML 註解裡的標題 | 讀者根本看不到 |
   | 只有標題、底下沒有內容 | 標題不是「改了什麼」的說明 |
   | 同一個版本兩個區段 | 檔案沒有說哪一個才是這次發布 |
   | 檔案裡有未收尾的柵欄或 `<!--` | 它後面的東西會被整段掃進內文，而 release 一旦發出就不能覆寫 |
   | raw HTML 區塊（`<pre>` 等）裡的標題 | 那是預格式化文字，讀者看到的不是標題 |
   | 內容只有 HTML 註解（`<!-- TODO -->`） | 渲染出來什麼都沒有，等於只有標題 |

   **已知限制**（方向都是誤拒，不會誤放行）：散文或行內程式碼裡出現字面的
   `<!--` 而沒有 `-->` 時，掃描會當成註解開始，整份檔案以「未收尾的註解」被
   拒絕。要分辨它需要解析行內程式碼，這支腳本不做。它會拒絕並說出理由，不會
   把東西發出去。

   **切 beta 的摩擦，先講清楚**：因為預發行也擋，`## Unreleased` 必須在推
   `v1.0.13-beta.1` **之前**就改名成 `## 1.0.13 — <日期>`。而這份檔案開頭說
   「日期是 build 的日期」，所以正式版那次通常要再改一次日期。這是摩擦不是
   阻擋 —— tag 本來就已經指定版本號了。

   跟 (a) 不同的是預發行也擋：走查只有一個人做得到，寫下改了什麼不是，而一份
   沒有說明改了什麼的 beta，對讀的人跟正式版一樣不透明。

   為什麼要有 (b)：`v1.0.12` 以前每一份 release 內文都只講這份 build **不是**
   什麼（誰簽的、走查到哪、還沒驗什麼），一個字沒講它**是**什麼。
   `CHANGELOG.md` 一直都寫著，只是沒有任何路徑把它送到 release 頁面上，而使用
   者是在 GitHub 上讀不到才說的。內文放英文原文。繁體中文版本維護在
   `store/metadata/zh-TW/changelogs/`，由 `store/upload.sh` 送到 Play 的
   「最新動態」—— 那是另一條發布線，**只發到 GitHub 的 beta 不會有對應的中文
   商店文案**。兩邊各自維護，腳本不做機器翻譯。
